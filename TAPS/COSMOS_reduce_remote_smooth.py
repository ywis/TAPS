...
def derive_1D_spectra_Av_corrected(OneD_1, redshift_1, rownumber, wave_list, band_list, photometric_flux, photometric_flux_err, photometric_flux_err_mod, A_v):
    ...
    # The returned values are in the rest frame
...
#range(30,len(df)):# for i in [x for x in range(len(df)-1,37,-1) if x != 61 and x !=70 and x!=75]:
    ...
    smoothing_deltal=23.25/(1+redshift_1)#*df_photometry[df_photometry.id==ID].b_image.values[0]#/(1+redshift_1)#787.245
...
#-------------------------------------------------Initial Reduce the spectra ----------------------------------------------------------
    ...
    FULL_DRZ_filename = '/home/siqi/COSMOS_WFC3_V4.1.5/cosmos-'+"{0:02d}".format(region)+'/cosmos-'+"{0:02d}".format(region)+'-F140W_drz_sci.fits'
...
# Test with the new BC03 models
    ...
    if redshift<0.49:
        try:
            ...
            pass 
    chi_square_list.to_csv('/home/siqi/TAPS/TAPS/outcome/numeric/chi_square_list_cosmos_'+str(date)+'_photo_'+str(region)+'_'+str(ID)+'.csv')
