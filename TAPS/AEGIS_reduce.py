...
df = pd.read_csv('/Volumes/My Passport/TPAGB/database/matching_galaxies_aegis_20200301_PSB.csv', sep=',')
...
def plot_1D_spectra_Av_corrected(x, y, y_err, z_NIR, wave_list, photometric_flux, photometric_flux_err, model1_wave, model1_flux, intrinsic_Av, region, ID, model_name='M05'):
    """
    input:
    * Redshift-corrected normed spectra
    * Redshift-corrected normed photometry
    
    Plot the observed spectra, photometry and the models
    """
    ...
    #plt.plot(model2_wave, model2_flux, color='g', linewidth=2, label='model 2')
    #plt.plot(wavelength_1Gyr_M13, Flux_1Gyr_M13/F_6000_1Gyr_M13, linewidth=2, color='#ff7f0e', label='Ma13, solar metallicity, 1 Gyr')    
    ...
    #plt.text(wave_list[1],0.3,'g',fontsize=12)
    #plt.text(wave_list[2],0.3,'r',fontsize=12)
    #plt.text(wave_list[3],0.3,'i',fontsize=12)
    #plt.text(wave_list[4],0.3,'z',fontsize=12)
    #plt.text(wave_list[5],0.4,'F606W',fontsize=12)
    #plt.text(wave_list[6],0.4,'F814W',fontsize=12)
    #plt.text(wave_list[7],0.35,'F125W',fontsize=12)
    #plt.text(wave_list[8],0.35,'F140W',fontsize=12)
    #plt.text(wave_list[9],0.35,'F160W',fontsize=12)
    #plt.text(wave_list[10],0.3,'J1',fontsize=12)
    #plt.text(wave_list[11],0.3,'J2',fontsize=12)
    #plt.text(wave_list[12],0.3,'J3',fontsize=12)
    #plt.text(wave_list[13],0.3,'H1',fontsize=12)
    #plt.text(wave_list[14],0.3,'H2',fontsize=12)
    #plt.text(wave_list[15],0.3,'K',fontsize=12)
    #plt.text(wave_list[16],0.4,'J',fontsize=12)
    #plt.text(wave_list[17],0.4,'H',fontsize=12)
    #plt.text(wave_list[18],0.4,'Ks',fontsize=12)

     # Select the right normalization band
    if redshift_1 <= 0.10:
        temp_norm_wave = wave_list[5]#/(1+redshift_1)
        #plt.axvline(temp_norm_wave,linewidth=2,color='b') #F606W
    elif redshift_1 <= 0.22:
        temp_norm_wave = wave_list[2]#/(1+redshift_1)
        #plt.axvline(temp_norm_wave,linewidth=2,color='b') #r
    elif redshift_1 <= 0.42:
        temp_norm_wave = wave_list[3]#/(1+redshift_1)
        #plt.axvline(temp_norm_wave,linewidth=2,color='b') #i
    elif redshift_1 <= 0.55:
        temp_norm_wave = wave_list[6]#/(1+redshift_1)
        #plt.axvline(temp_norm_wave,linewidth=2,color='b') #F814W
    else:
        temp_norm_wave = wave_list[4]#/(1+redshift_1)
        #plt.axvline(temp_norm_wave,linewidth=2,color='b') #i
    ...
    plt.xlim([3e3,1.5e4])
    plt.ylim([0.05, ymax])#plt.ylim([ymin,ymax])
    plt.xlabel(r'Wavelength $\rm \AA$',fontsize=20)
    plt.ylabel(r'$\rm F_{\lambda}/F_{0.55\mu m}$',fontsize=20)
    plt.tick_params(axis='both', which='major', labelsize=14)
    titlename='/Volumes/My Passport/GV_CMD_fn_table_20180904/AEGIS_352_237_20180911/20181101/aegis_GV_CMD'+"-G141_"+"{0:05d}".format(ID)+'-'+"{0:02d}".format(region)+'_'+model_name+".png"    
    #print(titlename)
    #==============Residual Plot========================
    frame2 = fig1.add_axes((.1,.1,.8,.2))  
    index0 = 0
    relative_residuals=np.zeros([2,n])
    for wave in x:
        if y[index0]>0.25 and y[index0]<1.35:
            ...
            #index = find_nearest(model2_wave, wave);#print index
            #relative_residuals[1, index0] = y[index0]/model2_flux[index]
            #index = find_nearest(wavelength_1Gyr_M13,wave);#print index
            #relative_residuals[2,index0]=y[index0]/(Flux_1Gyr_M13[index]/F_6000_1Gyr_M13)
            index0=index0+1
    if index0 > 0:
    ...
    for i in range(len(wave_list)):
        ...
        index = find_nearest(model1_wave, wave_list[i])
        #print('line 368:',len(model1_wave),index, index0, n)
        relative_photo[0,index0]=model1_flux[index]/(photometric_flux[i])
        plt.errorbar(wave_list[i], (photometric_flux[i])/model1_flux[index], yerr=photometric_flux_err[i]/model1_flux[index], fmt='o', color='r', markersize=16)
        index0 = index0+1
...
def plot_3models(x, y, y_err, z_NIR, wave_list, band_list, photometric_flux, photometric_flux_err, model1_wave, model1_flux, AV_prior_optimized_M05, \
    ...
    frame1 = fig1.add_axes((.1,.35,.8,.6))# [x0,y0,width,height]
    ...
    titlename='/Volumes/My Passport/GV_CMD_fn_table_20180904/AEGIS_352_237_20180911/20190711/aegis_GV_CMD'+"-G141_"+"{0:05d}".format(ID)+'-'+"{0:02d}".format(region)+'_three_models.pdf' 
...
# Photometry
    ...
    photometric_flux_err = np.array([u_err+0.1*u, g_err+0.1*g, r_err+0.1*r, i_err+0.1*r, z_err+0.1*z,\
...
chi_square_list.to_csv('/Volumes/My Passport/GV_CMD_fn_table_20180904/chi_square_list_aegis-three_models_optimized_20200301_scale_photo.csv')
