...
# ### Reading Spectra
df_cat=pd.read_csv('/home/siqi/goodsn_3dhst_v4.1.5_catalogs/goodsn_3dhst.v4.1.5.zbest.rf',delim_whitespace=True,header=None,comment='#',index_col=False)
...
# df = pd.read_csv('/home/siqi/TAPS/TAPS/source_list/matching_galaxies_goodsn_20200303_PSB.csv', sep=',')
df = pd.read_csv('/home/siqi/TAPS/TAPS/source_list/matching_galaxies_goodsn_20200317_PSB.csv', sep=',')
df.columns=['detector','ID','region','filename','chip']

df_photometry=pd.read_csv('/home/siqi/goodsn_3dhst.v4.1.cats/Catalog/goodsn_3dhst.v4.1.cat', delim_whitespace=True,header=None,comment='#',index_col=False)
...
df_fast = pd.read_csv('/home/siqi/goodsn_3dhst.v4.1.cats/Fast/goodsn_3dhst.v4.1.fout', delim_whitespace=True,header=None,comment='#',index_col=False)
...
def read_spectra(row):
    ...
    if detector == 'WFC3':
        filename="/home/siqi/GOODSN_WFC3_V4.1.5/goodsn-"+"{0:02d}".format(region)+"/1D/ASCII/goodsn-"+"{0:02d}".format(region)+"-G141_"+"{0:05d}".format(ID)+".1D.ascii"
        OneD_1 = np.loadtxt(filename,skiprows=1)
    if detector =="ACS":
        filename="/home/siqi/GOODSN_ACS_V4.1.5/acs-goodsn-"+"{0:02d}".format(region)+"/1D/FITS/"+df.filename[row]
...
def minimize_age_AV_vector_weighted_grizli(X):
    ...
    # 1. Read and generate model spectra
    n=len(x)
    age_index = find_nearest(df_Ma.Age.unique(), galaxy_age)
    age_prior = df_Ma.Age.unique()[age_index]
    AV_string = str(intrinsic_Av)
...
def plot_grizli_fit(X,beam_g141):
    ...
    figname=current_dir+outcome_dir+plot_dir+'Forward_Modelling_GOODSN_M05_'\
    ...
    figname=current_dir+outcome_dir+plot_dir+'GOODSN_M05_SSP_opt_'+str(region)+'_'+str(ID)+'_'+"{0:.2f}".format(X[0])+'Gyr_AV'+"{0:.2f}".format(X[1])+'_grizli.pdf'
    plt.savefig(figname)
    plt.clf()
...
def lg_minimize_age_AV_vector_weighted_M13_grizli(X):
    ...
    if np.isinf(lnprobval):
        print('lnprob:',lnprobval, chi2_nu_spec, x2_photo,galaxy_age,intrinsic_Av)
    return lnprobval
def minimize_age_AV_vector_weighted_M13_return_chi2_sep_grizli(X):
    from scipy.interpolate import interp1d
...
def plot_grizli_fit_M13(X,beam_g141):
    ...
    ax2.plot(beam_g141.beam.lam, beam_g141.beam.sensitivity, label='G141',linestyle='--',color='k')
    ax2.set_ylabel(r'Sensitivity[$\rm \frac{e^{-1}/s}{erg/s/cm^2}$]')
    figname=current_dir+outcome_dir+plot_dir+'Forward_Modelling_goodsn_M13_'\
                +str(region)+'_'+str(ID)+"_age_"+"{0:.1f}".format(galaxy_age)\
                +'_AV_'+"{0:.1f}".format(intrinsic_Av)+'.pdf'
    plt.savefig(figname)
    plt.clf()
    ...
    plt.tick_params(axis='both', which='major', labelsize=20)
    plt.tick_params(axis='both', which='minor', labelsize=20)
    figname=current_dir+outcome_dir+plot_dir+'GOODSN_M13_SSP_opt_'+str(region)+'_'+str(ID)+'_'+"{0:.2f}".format(X[0])+'Gyr_AV'+"{0:.2f}".format(X[1])+'_grizli.pdf'
    plt.savefig(figname)
    plt.clf()
...
def plot_grizli_fit_BC(X,beam_g141):
    ...
    ax2.plot(beam_g141.beam.lam, beam_g141.beam.sensitivity, label='G141',linestyle='--',color='k')
    ax2.set_ylabel(r'Sensitivity[$\rm \frac{e^{-1}/s}{erg/s/cm^2}$]')
    figname=current_dir+outcome_dir+plot_dir+'Forward_Modelling_GOODSN_BC_'\
                +str(region)+'_'+str(ID)+"_age_"+"{0:.1f}".format(galaxy_age)\
                +'_AV_'+"{0:.1f}".format(intrinsic_Av)+'.pdf'
    plt.savefig(figname)
    plt.clf()
    ...
    plt.tick_params(axis='both', which='major', labelsize=20)
    plt.tick_params(axis='both', which='minor', labelsize=20)
    figname=current_dir+outcome_dir+plot_dir+'GOODSN_BC_SSP_opt_'+str(region)+'_'+str(ID)+'_'+"{0:.2f}".format(X[0])+'Gyr_AV'+"{0:.2f}".format(X[1])+'_grizli.pdf'
    plt.savefig(figname)
    plt.clf()
...
def Scale_model(D, sig, M):
    """
    Scale *Model* to match with Data
    """
...
path = "/home/siqi/TAPS/TAPS/filter/goodsn/"
import glob, os
os.chdir(path)
for i in range(1,19):
...
GOODSN_FLT_list=[
                '/home/siqi/goodsn_flt_files/ib3701sdq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3702uhq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3703vaq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3704x3q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3717x0q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3718avq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3719v4q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3720fdq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3720fdq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3720fdq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3705yeq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3706bfq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3707cnq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3708igq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3721xvq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3722jiq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3723r0q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3724rvq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3724rvq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3724rvq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3709jlq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3710njq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3711coq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3712m7q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3725ebq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3726c2q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3726c2q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3726c2q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3726c2q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3726c2q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3713xaq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3714g9q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3715skq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3716q5q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3727u2q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3728dbq_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3744r1q_flt.fits',
                '/home/siqi/goodsn_flt_files/ib3747zbq_flt.fits',
                ]
...
# -----Capture the FLT file -----
    if region < 100:
        sim_g141 = grizli.model.GrismFLT(grism_file='', \
                                         direct_file=GOODSN_FLT_list[region-11],\
                                         pad=100,verbose=True)
    elif region == 114:
        sim_g141 = grizli.model.GrismFLT(grism_file='', \
                                         direct_file=GOODSN_FLT_list[-2],\
                                         pad=100,verbose=True)
    elif region ==123:
        sim_g141 = grizli.model.GrismFLT(grism_file='', \
                                         direct_file=GOODSN_FLT_list[-1],\
                                         pad=100,verbose=True)
    ...
    sim_cat = sim_g141.catalog
    sim_g141.compute_full_model(ids=sim_cat['id'][keep], mags=sim_cat['mag'][keep])
    source_ra = df_photometry.iloc[ID_no].ra
    source_dec = df_photometry.iloc[ID_no].dec
    sim_dr = np.sqrt((sim_cat['sky_centroid'].ra.value-source_ra)**2+\
                 (sim_cat['sky_centroid'].dec.value-source_dec)**2)    
    sim_ix = np.argmin(sim_dr)
    print(sim_ix)
    sim_id = sim_g141.catalog['id'][sim_ix]
    print(sim_id)
    x0 = sim_g141.catalog['x_flt'][sim_ix].value+1
    y0 = sim_g141.catalog['y_flt'][sim_ix].value+1
    print('id=%d, (x,y)=(%.1f, %.1f)' %(sim_id, x0, y0))
    beam_g141 = model.BeamCutout(sim_g141, 
                                 sim_g141.object_dispersers[sim_id][2]['A']) 
    sky_countrate = 1. # e-/s
    texp = 1000.       # s
    rn = 20.           # readnoise, e-
...
### grizli work in the virtual env
    ...
    band_list = band_list*(1+redshift_1)
    X = np.array([galaxy_age, intrinsic_Av])

# Using bounds to constrain
    print('____________________M05_________________________ Optimization Grizli__________________________')
    X = np.array([galaxy_age, intrinsic_Av])
    ...
    #     sampler = emcee.EnsembleSampler(nwalkers, ndim, lg_minimize_age_AV_vector_weighted_grizli, pool=pool)
    ...
    #         figname=current_dir+outcome_dir+plot_dir+"GOODSN_triangle_M05_"+str(nsteps)+'_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(X[0])+'Gyr_AV'+"{0:.2f}".format(X[1])+"_grizli.pdf"
    #         fig.savefig(figname)
    #         X = np.percentile(samples, 50, axis=0)
    #         x2_optimized = minimize_age_AV_vector_weighted_grizli(X)
    #         x2_spec, x2_phot = minimize_age_AV_vector_weighted_return_chi2_sep_grizli(X)
...
# Test with the new M13 models
    print('____________________M13_________________________ Optimization GRIZLI__________________________')
    ...
    #     sampler = emcee.EnsembleSampler(nwalkers, ndim, lg_minimize_age_AV_vector_weighted_M13_grizli, pool=pool)
    #     sampler.run_mcmc(p0,nsteps, progress=True)
    #     # print(np.size(samples))
    ...
    #                 ax.plot(value2[xi],value2[yi],'sr')
    #         plt.rcParams.update({'font.size': 12})
    #         figname=current_dir+outcome_dir+plot_dir+"GOODSN_triangle_M13_"+str(nsteps)+'_'+str(region)+'_'+str(ID)+'_'+"{0:.2f}".format(X[0])+'Gyr_AV'+"{0:.2f}".format(X[1])+"_grizli.pdf"
    #         fig.savefig(figname)
    #         fig.clf()
...
# Test with the new BC03 models
    ...
    #                 ax.plot(value2[xi],value2[yi],'sr')
    #         plt.rcParams.update({'font.size': 12})
    #         figname=current_dir+outcome_dir+plot_dir+"GOODSN_triangle_BC03_"+str(nsteps)+'_'+str(region)+'_'+str(ID)+'_'+"{0:.2f}".format(X[0])+'Gyr_AV'+"{0:.2f}".format(X[1])+"_grizli.pdf"
    #         plt.savefig(figname)
    #         plt.clf()
    ...
    #     pass
## Save to catalog
    # chi_square_list.to_csv('/home/siqi/TAPS/TAPS/outcome/numeric/chi_square_list_goodsn_'+str(date)+'_photo_'+str(region)+'_'+str(ID)+'_grizli.csv')
    # print('-------------------------------------End--------------------------------------------------------------------------------------')
chi_square_list.to_csv('/home/siqi/TAPS/TAPS/outcome/numeric/chi_square_list_goodsn_'+str(date)+'_photo_'+str(region)+'_'+str(ID)+'_grizli_all.csv')
print('-------------------------------------End--------------------------------------------------------------------------------------')
