...
def reduced_chi_square(data_wave,data,data_err,model_wave,model):
    ...
    chi_square = 0
    for i in range(n):
        index = find_nearest(model_wave,data_wave[i])
...
def plot_1D_spectra_Av_corrected(x, y, y_err, z_NIR, wave_list, photometric_flux, photometric_flux_err, model1_wave, model1_flux, intrinsic_Av, region, ID, model_name='M05'):
    ...
    plt.plot(model1_wave, model1_flux, color='k', linewidth=2, label='model 1')
    ...
    if np.max(y_err/y)<1:
        plt.fill_between(x,(y+y_err),(y-y_err),alpha=0.05)
    ...
    plt.errorbar(wave_list, photometric_flux, yerr=photometric_flux_err, color='r',fmt='o',label='photometric data',markersize='14')
    
    plt.text(wave_list[0],0.3,'U',fontsize=12)
    plt.text(wave_list[1],0.4,'B',fontsize=12)
    plt.text(wave_list[2],0.3,'V',fontsize=12)
    plt.text(wave_list[3],0.4,'R',fontsize=12)
    plt.text(wave_list[4],0.3,'i',fontsize=12)
    plt.text(wave_list[5],0.4,'z',fontsize=12)
    plt.text(wave_list[6],0.25,'F606W',fontsize=12)
    plt.text(wave_list[7],0.25,'F814W',fontsize=12)
    plt.text(wave_list[8],0.3,'F125W',fontsize=12)
    plt.text(wave_list[9],0.3,'F140W',fontsize=12)
    plt.text(wave_list[10],0.3,'F160W',fontsize=12)
    plt.text(wave_list[11],0.4,'J',fontsize=12)
    plt.text(wave_list[12],0.4,'H',fontsize=12)
    plt.text(wave_list[13],0.4,'K',fontsize=12)
    
    plt.text(1.2e4, 1.2, r'Galactic $A_V$='+"{0:.2f}".format(intrinsic_Av),fontsize=12)
    if redshift_1<=0.05:
        i = 2
        temp_norm_wave = wave_list[i]#/(1+redshift_1)
        #index_wave_norm = find_nearest(smooth_wavelength_BC,temp_norm_wave)
    ...
    plt.xlim([3000, 1.2*xmax])
    plt.ylim([0.25,ymax])#plt.ylim([ymin,ymax])
    plt.xlabel(r'wavelength $\rm \AA$',fontsize=20)
    plt.ylabel(r'$\rm F_{\lambda}/F_{0.55\mu m}$',fontsize=20)
    titlename='/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20181028/uds_GV_CMD'+ "-G141_"+"{0:05d}".format(ID)+'-'+"{0:02d}".format(region)+'_'+model_name+".png"
    
    #==============Residual Plot========================
    frame2=fig1.add_axes((.1,.1,.8,.2))  
    index0=0
    relative_residuals=np.zeros([3,n])
    for wave in x:
        if y[index0]>0.25 and y[index0]<1.35:
            index = find_nearest(model1_wave, wave);#print index
            relative_residuals[0, index0] = y[index0]/model1_flux[index]
            #print(wave,x[index0])
            index0=index0+1
    if index0 > 0:
        plt.plot(x[:index0],relative_residuals[0,:index0],color='k')
        #plt.plot(x[:index0],relative_residuals[1,:index0],color='g')
        #plt.plot(x[:index0],relative_residuals[2,:index0],color='#ff7f0e')        
        
    index0=0
    relative_photo = np.zeros([1,n])+1
    for i in range(len(wave_list)):
        if wave_list[i]>np.min(x) and wave_list[i]<np.max(x):
            index = find_nearest(x, wave_list[i])
            relative_photo[0,index0]=y[index]/(photometric_flux[i])
            plt.errorbar(wave_list[i], y[index]/(photometric_flux[i]), yerr=y_err[index]/(photometric_flux[i]), fmt='o', color='r', markersize=16)
            index0=index0+1
    relative_residuals[np.isnan(relative_residuals)] = 0
    plt.axhline(1.0, linewidth=2, color='k') 
    plt.xlim([3000, 1.2*xmax])
    #print(relative_residuals)
    plt.ylim([max(0,np.min(relative_photo)-0.2), np.max(relative_residuals)+0.1])# this is the situation for 
...
def plot_3models(x, y, y_err, z_NIR, wave_list, band_list, photometric_flux, photometric_flux_err, model1_wave, model1_flux, AV_prior_optimized_M05, \
    ...
    plt.xlim([2.5e3,1.5e4])
    plt.ylim([0.1, 1.4])#plt.ylim([ymin,ymax])
    plt.xlabel(r'Wavelength $\rm \AA$',fontsize=24)
    plt.ylabel(r'$\rm F_{\lambda}/F_{0.55\mu m}$',fontsize=24)
    plt.tick_params(axis='both', which='major', labelsize=22)
    plt.legend(loc='upper right',fontsize=24)
    titlename='/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20190715/uds_GV_CMD'+ "-G141_"+"{0:05d}".format(ID)+'-'+"{0:02d}".format(region)+"_three_models.pdf"
    ...
    plt.ylim([0.5, 1.5])#max(np.max(relative_residual_1)+0.1,2.0)])# this is the situation for 
    #plt.xlabel(r'Wavelength($\rm \AA$)', fontsize=20)
    #plt.ylabel('Relative Residual', fontsize=14) #Flux (spectra over model/photometry)
...
#-------------------------------------------------Initial Reduce the spectra ----------------------------------------------------------
    print('-------------------------------------Initial fit ---------------------------------------------------------------------------------------')
    [x, y, y_err, wave_list, band_list, photometric_flux, photometric_flux_err] = derive_1D_spectra_Av_corrected(OneD_1, redshift_1, row, wave_list, band_list, photometric_flux, photometric_flux_err, A_v)
    if redshift< 0.49:
        try:
...
# Using bounds to constrain
    ...
    figname=current_dir+outcome_dir+plot_dir+'UDS_SSP_M05_opt_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf'
    plt.savefig(figname)
    plt.clf()
    # plt.savefig('/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/UDS_SSP_M05_opt_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf')
    ...
    try:   
        if samples.size > 1e3:
            ...
            figname=current_dir+outcome_dir+plot_dir+"uds_triangle_M05_"+str(nsteps)+'_'+str(ID)+'_'+str(region)+".pdf"
            fig.savefig(figname)
            # fig.savefig("/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/uds_triangle_M05_"+str(nsteps)+'_'+str(ID)+'_'+str(region)+".pdf")
            fig.clf()
            ...
            plt.tick_params(axis='both', which='major', labelsize=20)
            plt.tick_params(axis='both', which='minor', labelsize=20)
            figname=current_dir+outcome_dir+plot_dir+'UDS_SSP_M05_MCMC50_'+str(nsteps)+'_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf'
            plt.savefig(figname)
            plt.clf()
            # plt.savefig('/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/UDS_SSP_M05_MCMC50_'+str(nsteps)+'_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf')
        else:
            with Pool() as pool:
                ndim, nwalkers = 2, 10
                tik = time.clock()
...
# Test with the new M13 models
    ...
    plt.tick_params(axis='both', which='minor', labelsize=20)
    plt.xlabel(r'Wavelength($\rm \AA$)', fontsize=20)
    figname=current_dir+outcome_dir+plot_dir+'UDS_SSP_M13_opt_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol_M13.x[0])+'Gyr_AV'+"{0:.2f}".format(sol_M13.x[1])+'-SSP.pdf'
    plt.savefig(figname)
    plt.clf()
    # plt.savefig('/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/UDS_SSP_M13_opt_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf')
        ...
        if samples.size > 1e3 :
            ...
            for yi in range(ndim):
                for xi in range(yi):
                    ...
                    ax.plot(value2[xi],value2[yi],'sr')
            plt.rcParams.update({'font.size': 12})
            figname=current_dir+outcome_dir+plot_dir+"uds_triangle_M13_"+str(nsteps)+'_'+str(ID)+'_'+str(region)+".pdf"
            fig.savefig(figname)
            # fig.savefig("/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/uds_triangle_M13_"+str(nsteps)+'_'+str(ID)+'_'+str(region)+".pdf")
            fig.clf()
            ...
            plt.tick_params(axis='both', which='major', labelsize=20)
            plt.tick_params(axis='both', which='minor', labelsize=20)
            figname=current_dir+outcome_dir+plot_dir+'UDS_SSP_M13_MCMC50_'+str(nsteps)+'_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol_M13.x[0])+'Gyr_AV'+"{0:.2f}".format(sol_M13.x[1])+'-SSP.pdf'
            plt.savefig(figname)
            plt.clf()
            # plt.savefig('/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/UDS_SSP_M13_MCMC50_'+str(nsteps)+'_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf')
        else:
            with Pool() as pool:
                ndim, nwalkers = 2, 10
                tik = time.clock()
...
# Test with the new BC03 models
    ...
    plt.tick_params(axis='both', which='major', labelsize=20)
    plt.tick_params(axis='both', which='minor', labelsize=20)
    figname=current_dir+outcome_dir+plot_dir+'UDS_SSP_BC03_opt_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol_BC03.x[0])+'Gyr_AV'+"{0:.2f}".format(sol_BC03.x[1])+'-SSP.pdf'
    plt.savefig(figname)
    plt.clf()
    # plt.savefig('/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/UDS_SSP_BC03_opt_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf')
        ...
        if samples.size > 1e3:
            ...
            for yi in range(ndim):
                for xi in range(yi):
                    ...
                    ax.plot(value2[xi],value2[yi],'sr')
            plt.rcParams.update({'font.size': 12})
            figname=current_dir+outcome_dir+plot_dir+"uds_triangle_BC03_"+str(nsteps)+'_'+str(ID)+'_'+str(region)+".pdf"
            fig.savefig(figname)
            # fig.savefig("/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/uds_triangle_BC03_"+str(nsteps)+'_'+str(ID)+'_'+str(region)+".pdf")
            fig.clf()
            ...
            plt.tick_params(axis='both', which='major', labelsize=20)
            plt.tick_params(axis='both', which='minor', labelsize=20)
            figname=current_dir+outcome_dir+plot_dir+'UDS_SSP_BC03_MCMC50_'+str(nsteps)+'_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol_BC03.x[0])+'Gyr_AV'+"{0:.2f}".format(sol_BC03.x[1])+'-SSP.pdf'
            plt.savefig(figname)
            plt.clf()
            # plt.savefig('/Volumes/My Passport/GV_CMD_fn_table_20180904/UDS_516_337_20180911/20200207/UDS_SSP_BC03_MCMC50_'+str(nsteps)+'_'+str(ID)+'_'+str(region)+'_'+"{0:.2f}".format(sol.x[0])+'Gyr_AV'+"{0:.2f}".format(sol.x[1])+'-SSP.pdf')
        else:
            with Pool() as pool:
                ndim, nwalkers = 2, 10
                tik = time.clock()
